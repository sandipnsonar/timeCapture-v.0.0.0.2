...
//        ArrayList<HashMap<String, String>> dataList = new ArrayList<>();
//        SQLiteDatabase db = this.getReadableDatabase();
//        Cursor cursor = db.rawQuery("SELECT " + COLUMN_DATETIME + ", " + COLUMN_LATITUDE + ", " + COLUMN_LONGITUDE + " FROM " + TABLE_NAME, null);
//
//        if (cursor.moveToFirst()) {
//            do {
//                HashMap<String, String> data = new HashMap<>();
//                data.put(COLUMN_DATETIME, cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_DATETIME)));
//                data.put(COLUMN_LATITUDE, cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_LATITUDE)));
//                data.put(COLUMN_LONGITUDE, cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_LONGITUDE)));
//                dataList.add(data);
//            } while (cursor.moveToNext());
...
//        HashMap<String, String> lastLocation = null;
//        SQLiteDatabase db = this.getReadableDatabase();
//        Cursor cursor = db.rawQuery("SELECT * FROM " + TABLE_NAME + " ORDER BY " + COLUMN_DATETIME + " DESC LIMIT 1", null);
//
//        if (cursor != null && cursor.moveToFirst()) {
//            lastLocation = new HashMap<>();
//            lastLocation.put(COLUMN_DATETIME, cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_DATETIME)));
//            lastLocation.put(COLUMN_LATITUDE, cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_LATITUDE)));
//            lastLocation.put(COLUMN_LONGITUDE, cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_LONGITUDE)));
//            cursor.close();
//        }
//
//        db.close();
